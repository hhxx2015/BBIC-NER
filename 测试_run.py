...
def trymask(string):
    ...
    for i in range(len(pred_ids_result[0][0])):
        if pred_ids_result[0][0][i]==19 or pred_ids_result[0][0][i]==1:
            pos.append(i)
    for i in range(1,len(pos)-1):
        pred_ids_result[0][0][pos[i]]=12
    # print(pred_ids_result[0][0])
...
if __name__ == "__main__":
    ...
    yuceacc_dic=  {'DIS':0,'SYM':0,'PRO':0,'EQU':0,'DRU':0,'ITE':0,'BOD':0,'DEP':0,'MIC':0}
    yuce_dic = {'DIS': 0, 'SYM': 0, 'PRO': 0, 'EQU': 0, 'DRU': 0, 'ITE': 0, 'BOD': 0, 'DEP': 0, 'MIC': 0}
    quan_dic=  {'DIS':0,'SYM':0,'PRO':0,'EQU':0,'DRU':0,'ITE':0,'BOD':0,'DEP':0,'MIC':0}
    with graph.as_default():
        with open("data/val_data.txt",'r',encoding='utf-8') as f:
            ...
            for line in f:
                try:
                    string,daan_list = getlinepostion(line.rstrip('\n'))
                    string = Converter('zh-hans').convert(string)
                    print(string)
                    for z in daan_list:
                        quan_dic[z[0]]+=1
                        print(z,string[z[1]:z[2]+1],'CRR')
                    # print(daan_list)
                    print('------------------------')
                    # 吧string切片送进去
                    # string=[]

                    ans=fenpian(string,n=MAX_N)
                    anss=ans
                    # 211 III/BII
                    ...
                    # print(anss)
                    # print(anss)
                    res=maskrecon2(anss,n=MAX_N)
                    # print(res)
                    yuce_list = tiaozheng(res)
                    # print(yuce_list)
                    yuc = []
                    for y in yuce_list:
                        if y[-1]!=y[-2]:
                    ...
                    ans_count1=len(daan_list)
                    yuc_count1=len(yuce_list)
                    ...
                    for i in yuce_list:
                        if i in daan_list:
                            yuceacc_dic[i[0]]+=1
                            acc_count1+=1
                    err_count1 = yuc_count1-acc_count1
                    ig_count1 = ans_count1-acc_count1
                    print("line:%d acc_count:%d err_count:%d ig_count:%d"%(i_ccount,acc_count1,err_count1,ig_count1))
                    acc_count = acc_count1+acc_count
                    err_count = err_count1+err_count
                    ig_count = ig_count1+ig_count
                except:
                    with open('error22.txt','a+',encoding='utf-8') as ff:
                        ff.write(line)
                def ac(a,b):
                    c=a+b+0.00000000001 if a+b==0 else a+b
                    return a*1.0/c
                def rcal(a,b):
                    c = a + b + 0.00000000001 if a + b == 0 else a + b
                    return a * 1.0 / c
                def F1(a,b):
                    c = a + b + 0.00000000001 if a + b == 0 else a + b
                    return 2.0*a*b/c
                print("Acc_count:%d Err_count:%d Ig_count:%d" % (acc_count, err_count, ig_count))
                precision = ac(acc_count,err_count)
                recall = rcal(acc_count,ig_count)
                ...
                p_mic=[]
                rc_mic=[]
                for h in ['DIS', 'SYM', 'PRO', 'EQU', 'DRU', 'ITE', 'BOD', 'DEP', 'MIC']:
                    acc_counttt=yuceacc_dic[h]
                    err_counttt=yuce_dic[h]-acc_counttt
                    ig_counttt=quan_dic[h]-acc_counttt
                    p_mic.append(ac(acc_counttt,err_counttt))
                    rc_mic.append(rcal(acc_counttt,ig_counttt))
                print(p_mic)
                print(rc_mic)
                # print('Macro',sum(p_mic)/9.0,sum(rc_mic)/9.0,F1(sum(p_mic)/9.0,sum(rc_mic)/9.0))
                # print('F1_Micro_Macro',F1(f1,F1(sum(p_mic)/9.0,sum(rc_mic)/9.0)))
                print('================================================')
